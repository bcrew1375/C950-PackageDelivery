...
def deliver_packages():
    global number_of_packages_left
    ...
    while truck_1.get_number_loaded() > 0:
        ...
        for i in range(0, truck_1.get_number_loaded(), 1):
            ...
            if earliest_deadline is None:
                earliest_deadline = convert_timestamp_to_minutes(
                ...
                # If two deadlines are the same, determine which has the shortest distance.
                    ...
                    distance_2 = distance_table.get_distance(from_address,
                                                             parcel_table.search(parcel_id).get_parcel_address())
...
def load_trucks_at_hub():
    if truck_1.get_at_hub() is True or truck_2.get_at_hub() is True:
        ...
        for i in range(0, len(truck_1_package_list), 1):
            ...
            if (current_parcel.get_parcel_id() in truck_1_package_list) and (
                    truck_1.get_number_loaded() < truck_1.get_capacity()) \
        ...
        for i in range(0, len(truck_2_package_list), 1):
            ...
            if (current_parcel.get_parcel_id() in truck_2_package_list) and (
                    truck_2.get_number_loaded() < truck_2.get_capacity()) \
...
def run_delivery_algorithm():
    ...
    while number_of_packages_left > 0:
        ...
        # Set the departure time of each loaded package.
        for i in range(0, truck_1.get_number_loaded(), 1):
